...
int set_up_check_permutation_memory(
  ...
  if (!allocated || setupRows != rows || setupCols != cols ||
    ...
    gpuErrchk(cudaMalloc((void**)dpermutations, num_permutations * group_width *
...
extern "C" int run_check_permutations(
    ...
    unsigned int
        num_permutations  // array of permutations to try, group_width*4 values
                          // per each of num_permutations permutations
    ,
    float* improvement,
    unsigned int* permutation  // improvement offered by the best permutation,
                               // the best permutation
    ,
    unsigned int blocks, unsigned int threads  // blocks/threads per launch
) {
  threads = 32;
...
__global__ void swap_columns_sum_after_2_to_4(float* matrix, unsigned int rows,
  ...
  *base_psum = 0.0f;
  for (unsigned int s = 0; s < 16; ++s) local_psum[s] = 0.0f;
...
int run_build_swap_map(float* matrix, unsigned int rows, unsigned int cols,
                       unsigned int* stripe_pairs, unsigned int num_pairs,
                       float* output) {
...
/**********************************************************
 * Build the permute map for optimize_stripe_groups
 **********************************************************/
// optimize a stripe group, exhaustively searching all num_permutations provided
// permutations build the entire permute map at once each block handles one
// group of stripes each threads in the block handle all handle the same
// permutation at the same time on different rows before moving to the next
// permutation
...
void set_up_permute_map_memory(
  ...
  if (setUpNumPerms < num_permutations || setUpPermLength < perm_length) {
    ...
    gpuErrchk(cudaMalloc((void**)dpermutations, perm_length * num_permutations *
                                                    sizeof(unsigned int)));
  }
...
int run_build_permute_map(float* matrix, unsigned int rows, unsigned int cols,
                          unsigned int* stripes, unsigned int num_groups,
                          unsigned int group_width, unsigned int* permutations,
                          unsigned int num_permutations,
                          unsigned int perm_length, float* improvements,
                          unsigned int* best_indices) {
  static float* d_matrix = NULL;
